 ...
 */
public class SkyzButton extends ClickableWidget {
     ...
     */
    private void drawCenteredIconLabel(DrawContext ctx, TextRenderer tr,
        ...
        int iconW = icon.isEmpty() ? 0 : tr.getWidth(icon);
        int gap   = (icon.isEmpty() || text.isEmpty()) ? 0 : 5;
        int totalW = iconW + gap + textW;

        int sx = cx - totalW / 2;
        if (!icon.isEmpty()) {
            ctx.drawTextWithShadow(tr, icon, sx, y, colour);
            sx += iconW + gap;
        }
