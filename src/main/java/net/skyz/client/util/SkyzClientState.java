...
public final class SkyzClientState {
    ...
    private static final java.util.Set<net.minecraft.util.math.BlockPos> SPAWNER_PINGED =
            new java.util.HashSet<>();
    private static int spawnerScanTick = 0;
    private static net.minecraft.world.World spawnerLastWorld = null;
    ...
    public static final class BlockEspGroup {
        ...
        public final net.minecraft.block.Block[] blocks;
        public final boolean isBlockEntity; // auto-detected at construction
        public boolean enabled;
        public BlockEspGroup(String name, int color, net.minecraft.block.Block... blocks) {
            this.name = name; this.color = color; this.blocks = blocks;
            // Auto-detect: if the first block is BlockEntityProvider, use BE path.
            // All blocks in a group should be the same kind in practice (we group by category).
            this.isBlockEntity = blocks.length > 0
                    && blocks[0] instanceof net.minecraft.block.BlockEntityProvider;
        }
    }

    public record BlockEspMatch(net.minecraft.util.math.BlockPos pos, int color) {}
    ...
    static {
        ...
        BLOCK_ESP_GROUPS.add(new BlockEspGroup("Ender Chest",   0x00CC88,
                net.minecraft.block.Blocks.ENDER_CHEST));
        BLOCK_ESP_GROUPS.add(new BlockEspGroup("Hopper",        0x666666,
                net.minecraft.block.Blocks.HOPPER));
        BLOCK_ESP_GROUPS.add(new BlockEspGroup("Furnace",       0xFF8844,
                net.minecraft.block.Blocks.FURNACE, net.minecraft.block.Blocks.BLAST_FURNACE,
                net.minecraft.block.Blocks.SMOKER));
        BLOCK_ESP_GROUPS.add(new BlockEspGroup("Beacon",        0x88FFFF,
                net.minecraft.block.Blocks.BEACON));
        BLOCK_ESP_GROUPS.add(new BlockEspGroup("Conduit",       0x44CCFF,
                net.minecraft.block.Blocks.CONDUIT));
        BLOCK_ESP_GROUPS.add(new BlockEspGroup("Beehive",       0xFFCC44,
                net.minecraft.block.Blocks.BEEHIVE, net.minecraft.block.Blocks.BEE_NEST));
        BLOCK_ESP_GROUPS.add(new BlockEspGroup("End Portal Frame", 0xAA88FF,
                net.minecraft.block.Blocks.END_PORTAL_FRAME));
        BLOCK_ESP_GROUPS.add(new BlockEspGroup("Sculk Catalyst", 0x0088AA,
                net.minecraft.block.Blocks.SCULK_CATALYST, net.minecraft.block.Blocks.SCULK_SHRIEKER));
        BLOCK_ESP_GROUPS.add(new BlockEspGroup("Mob Heads",     0xFFEEDD,
                net.minecraft.block.Blocks.SKELETON_SKULL,
                net.minecraft.block.Blocks.WITHER_SKELETON_SKULL,
                net.minecraft.block.Blocks.PLAYER_HEAD,
                net.minecraft.block.Blocks.ZOMBIE_HEAD,
                net.minecraft.block.Blocks.CREEPER_HEAD,
                net.minecraft.block.Blocks.DRAGON_HEAD,
                net.minecraft.block.Blocks.PIGLIN_HEAD));
        BLOCK_ESP_GROUPS.add(new BlockEspGroup("Crafter",       0xAA7755,
                net.minecraft.block.Blocks.CRAFTER));
    ...
    private static net.minecraft.world.World blockEspLastWorld = null;
    ...
    public static void tick(MinecraftClient client) {
        ...
        // Autoclicker — fires left-click attack at the configured CPS while
        // the user is holding attack and looking at a mob. Mob-only gate
        // keeps it from spamming blocks (which would also break them).
        if (autoclicker && client.options.attackKey.isPressed()
                && client.targetedEntity != null
                && client.targetedEntity instanceof net.minecraft.entity.LivingEntity
     ...
     */
    private static void tickSpawnerAlert(MinecraftClient client) {
        ...
        net.minecraft.util.math.ChunkPos pCp = client.player.getChunkPos();
        ...
        for (int dx = -CHUNK_R; dx <= CHUNK_R; dx++) {
            for (int dz = -CHUNK_R; dz <= CHUNK_R; dz++) {
                ...
                java.util.Map<net.minecraft.util.math.BlockPos,
                        net.minecraft.block.entity.BlockEntity> bes = chunk.getBlockEntities();
                if (bes.isEmpty()) continue;
                for (java.util.Map.Entry<net.minecraft.util.math.BlockPos,
                        net.minecraft.block.entity.BlockEntity> e : bes.entrySet()) {
                    net.minecraft.block.entity.BlockEntity be = e.getValue();
                    ...
                    net.minecraft.util.math.BlockPos pos = e.getKey();
     ...
     */
    private static String classifyAlertBlock(net.minecraft.block.Block block,
                                             net.minecraft.block.BlockState state) {
        if (block == net.minecraft.block.Blocks.SPAWNER)        return "Spawner";
        if (block == net.minecraft.block.Blocks.TRIAL_SPAWNER)  return "Trial Spawner";
        if (block == net.minecraft.block.Blocks.VAULT) {
            ...
            // if Yarn renames the constant.
            for (net.minecraft.state.property.Property<?> p : state.getProperties()) {
                if (p.getName().equals("ominous")
                        && p instanceof net.minecraft.state.property.BooleanProperty bp) {
                    if (state.get(bp)) return "Ominous Vault";
                    return null;
                }
            }
        }
    ...
    private static void pingSpawner(MinecraftClient client,
     ...
     */
    private static void applyDynamicFps(MinecraftClient client) {
        var maxFpsOpt = client.options.getMaxFps();
        if (!dynamicFps) {
            if (savedMaxFps != -1) {
                try { maxFpsOpt.setValue(savedMaxFps); } catch (Exception ignored) {}
                savedMaxFps = -1;
            }
            return;
        }
        boolean focused = client.isWindowFocused();
        int current = maxFpsOpt.getValue();
        if (!focused && current > 30) {
            if (savedMaxFps == -1) savedMaxFps = current;
            try { maxFpsOpt.setValue(30); } catch (Exception ignored) {}
     ...
     */
    public static void onChatMessage(MinecraftClient client, String text) {
        if (!autoGG || client.player == null || client.getNetworkHandler() == null) return;
        long now = System.currentTimeMillis();
     ...
     * sections directly — ClientWorld chunks are immutable-ish after load
    ...
    private static void doBlockEspScan() {
        ...
        net.minecraft.util.math.ChunkPos pCp;
        try { pCp = client.player.getChunkPos(); }
        ...
        for (int dx = -R; dx <= R; dx++) {
            for (int dz = -R; dz <= R; dz++) {
                net.minecraft.world.chunk.WorldChunk chunk;
                try {
                    chunk = world.getChunkManager().getWorldChunk(pCp.x + dx, pCp.z + dz, false);
                ...
                if (!beGroups.isEmpty()) {
                    try {
                        for (java.util.Map.Entry<net.minecraft.util.math.BlockPos,
                                net.minecraft.block.entity.BlockEntity> e : chunk.getBlockEntities().entrySet()) {
                            net.minecraft.block.Block block = e.getValue().getCachedState().getBlock();
                            for (BlockEspGroup g : beGroups) {
                                boolean matched = false;
                                for (net.minecraft.block.Block target : g.blocks) {
                                    if (block == target) {
                ...
                if (!bsGroups.isEmpty()) {
                    try {
                        ...
                        for (int si = 0; si < sections.length; si++) {
                            net.minecraft.world.chunk.ChunkSection sec = sections[si];
                            ...
                            for (BlockEspGroup g : bsGroups) {
                                for (net.minecraft.block.Block tb : g.blocks) {
                                    if (sec.hasAny(s -> s.isOf(tb))) { hasTarget = true; break; }
                                }
                                if (hasTarget) break;
                            }
                            ...
                            for (int x = 0; x < 16; x++) for (int y = 0; y < 16; y++) for (int z = 0; z < 16; z++) {
                                net.minecraft.block.BlockState s = sec.getBlockState(x, y, z);
                                    ...
                                    for (net.minecraft.block.Block target : g.blocks) {
                                        if (b == target) {
                                            out.add(new BlockEspMatch(
                                                    new net.minecraft.util.math.BlockPos(baseChunkX + x, baseY + y, baseChunkZ + z),
                                                    g.color));
                                            matched = true;
                                            break;
                                        }
        ...
        if (out.size() > 2000) {
            net.minecraft.util.math.Vec3d pp = new net.minecraft.util.math.Vec3d(
                    client.player.getX(), client.player.getY(), client.player.getZ());
            out.sort((a, b) -> Double.compare(
                    a.pos().getSquaredDistance(pp), b.pos().getSquaredDistance(pp)));
