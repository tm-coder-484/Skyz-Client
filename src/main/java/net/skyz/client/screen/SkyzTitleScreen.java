 ...
 *     click dispatch via the wirings in {@link #build}.
 *   • This class still renders, by hand, everything that needs DrawContext
...
public class SkyzTitleScreen extends BaseUIModelScreen<FlowLayout> {
    ...
    public SkyzTitleScreen() {
        super(FlowLayout.class, Identifier.of("skyz_client", "title"));
    ...
    private void drawLogo(DrawContext ctx) {
        ...
        int letterSpacing  = 3;
        int totalUnscaledW = SkyzRenderHelper.textWidthSpaced(textRenderer, logo, letterSpacing);
        ...
        for (int[] off : offsets) {
            SkyzRenderHelper.drawTextSpaced(ctx, textRenderer, logo,
        ...
        SkyzRenderHelper.drawTextSpacedCentered(ctx, textRenderer, tagline, cx, taglineY,
                1, 0xCCB4E6FF, true);
    ...
    //    render Screen.addDrawableChild widgets) ──────────────────────────
    ...
    private void drawMuteButton(DrawContext ctx, int mx, int my) {
        ...
        int w = textRenderer.getWidth(icon);
        ctx.drawText(textRenderer, icon, cx - w / 2, cy - 4, hover ? 0xFFFFFFFF : 0xCCCCEFFF, false);
    }
