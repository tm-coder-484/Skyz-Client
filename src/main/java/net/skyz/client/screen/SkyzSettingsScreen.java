 ...
 *       with the theme's BG1→BG3 gradient + accent stripe. MouseButtonEvent to apply.</li>
 ...
 * with label/renderer that flips on MouseButtonEvent). Sliders use owo's built-in
...
public class SkyzSettingsScreen extends BaseUIModelScreen<FlowLayout> {
    ...
    public SkyzSettingsScreen(SkyzTitleScreen parent) {
        super(FlowLayout.class, Identifier.fromNamespaceAndPath("skyz_client", "settings"));
        this.parent = parent;
    }
    ...
    private FlowLayout buildThemeCard(int idx) {
        ...
        // Theme name.
        LabelComponent name = UIComponents.label(Component.literal(t[0]))
                .color(Color.ofArgb(selected ? 0xFFFFFFFF : 0xCCDDEEFF));
        name.margins(Insets.top(8));
        card.child(name);

        // MouseButtonEvent-to-apply via mouseDown on the card itself (the card isn't a
    ...
    private FlowLayout buildInterfaceSection() {
        ...
        sec.child(buildIntSliderRow("FOV", "°",
                options.fov().get(), 30, 110,
                v -> { options.fov().set(v); options.save(); }));
        ...
        sec.child(buildIntSliderRow("Brightness", "%",
                (int) Math.round(options.gamma().get() * 100), 0, 100,
                v -> { options.gamma().set(v / 100.0); options.save(); }));
        return sec;
    }
    ...
    private FlowLayout buildBlockEspRow(SkyzClientState.BlockEspGroup g) {
        ...
        row.child(UIComponents.label(Component.literal(g.name))
    ...
    private FlowLayout buildInfoRow(String name, String value) {
        ...
        row.child(UIComponents.label(Component.literal(value))
    ...
    private FlowLayout buildIntSliderRow(String name, String unit,
        ...
        topLine.child(UIComponents.label(Component.literal(name))
                .color(Color.ofArgb(SkyzColors.TEXT_PRIMARY))
                .horizontalSizing(Sizing.expand(100)));
        LabelComponent valueLbl = UIComponents.label(Component.literal(initial + " " + unit))
        ...
        slider.onChanged().subscribe(value -> {
            ...
            valueLbl.text(Component.literal(v + " " + unit));
            onChange.accept(v);
        });
        row.child(slider);
        return row;
     ...
     * makePill — same look, same flip-on-MouseButtonEvent behaviour.
     */
