 ...
 * shutdown), and a {@link MinecraftClient#execute(Runnable)} hop back to
...
public class SkyzShadersScreen extends BaseUIModelScreen<FlowLayout> {
    ...
    public SkyzShadersScreen(SkyzTitleScreen parent) {
        super(FlowLayout.class, Identifier.of("skyz_client", "shaders"));
        this.parent = parent;
    }
    ...
    protected void build(FlowLayout root) {
        ...
        if (irisStatusLabel != null) {
            irisStatusLabel.text(Text.literal(HAS_IRIS ? "🌅 Iris Active" : "⚠ Iris not installed"));
     ...
     * via {@link MinecraftClient#execute(Runnable)} to mutate owo state.
     *
    ...
    private FlowLayout buildPlaceholderRow(String text) {
        ...
        row.child(UIComponents.label(Text.literal(text))
    ...
    private FlowLayout buildPackCard(int idx, ShaderPack p) {
        ...
        card.child(UIComponents.label(Text.literal(icon))
        ...
        col.gap(2);
        col.child(UIComponents.label(Text.literal(truncate(p.name(), 26)))
                .color(Color.ofArgb(p.isEnabled() ? 0xFF8CD2FF : SkyzColors.TEXT_PRIMARY)));
        col.child(UIComponents.label(Text.literal(p.isEnabled() ? "✦ Active" : truncate(p.fileName(), 30)))
        ...
        ButtonComponent apply = UIComponents
                .button(Text.literal(p.isEnabled() ? "Active" : "Apply"),
        ...
        // Click anywhere on the card to select for the right-pane info.
        final int row = idx;
    ...
    private void rebuildInfoPanel() {
        ...
        infoPanel.child(UIComponents.label(Text.literal("PACK INFO"))
        ...
        if (sel != null) {
            infoPanel.child(UIComponents.label(Text.literal(sel.name()))
                    .color(Color.ofArgb(0xFF8CD2FF)));
            infoPanel.child(UIComponents.label(Text.literal("File: " + sel.fileName()))
                    .color(Color.ofArgb(0x4D8CD2FF)));
            infoPanel.child(UIComponents.label(Text.literal(
        ...
        if (!HAS_IRIS) {
            ...
            warn.child(UIComponents.label(Text.literal("⚠ Iris Shaders not installed"))
                    .color(Color.ofArgb(0xFFFF9944)));
            warn.child(UIComponents.label(Text.literal("To use shader packs, install:"))
                    .color(Color.ofArgb(SkyzColors.TEXT_MUTED)));
            warn.child(UIComponents.label(Text.literal("Iris + Sodium from modrinth.com"))
                    .color(Color.ofArgb(0x668CD2FF)));
            warn.child(UIComponents.label(Text.literal("modrinth.com/mod/iris"))
                    .color(Color.ofArgb(0x558CD2FF)));
            infoPanel.child(warn);
        } else {
            infoPanel.child(UIComponents.label(Text.literal("Shaderpacks folder:"))
                    .color(Color.ofArgb(SkyzColors.TEXT_MUTED)));
            String path = new File(MinecraftClient.getInstance().runDirectory, "shaderpacks")
                    .getAbsolutePath();
            infoPanel.child(UIComponents.label(Text.literal(truncate(path, 60)))
                    .color(Color.ofArgb(0x558CD2FF)));

            FlowLayout gap2 = UIContainers.verticalFlow(Sizing.fill(100), Sizing.fixed(6));
            infoPanel.child(gap2);

            infoPanel.child(UIComponents.label(Text.literal("Quick tips:"))
                    .color(Color.ofArgb(SkyzColors.TEXT_MUTED)));
            infoPanel.child(UIComponents.label(Text.literal("· Drop .zip files into shaderpacks/"))
                    .color(Color.ofArgb(0x778CD2FF)));
            infoPanel.child(UIComponents.label(Text.literal("· Use 'Refresh' to reload the list"))
                    .color(Color.ofArgb(0x778CD2FF)));
            infoPanel.child(UIComponents.label(Text.literal("· 'Open Full Iris Screen' for full settings"))
                    .color(Color.ofArgb(0x778CD2FF)));
        }
    }
    ...
    private boolean openIrisShaderScreen() {
        ...
        try {
            ...
            MinecraftClient.getInstance().setScreen(shaderScreen);
            return true;
        } catch (Exception e) {
