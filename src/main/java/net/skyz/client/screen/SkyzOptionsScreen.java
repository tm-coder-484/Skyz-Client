 ...
 * {@code SimpleOption<T>}-driven settings keep working untouched.
 *
...
public class SkyzOptionsScreen extends BaseUIModelScreen<FlowLayout> {
     ...
     * play, where Sodium's mixin on the Options screen swaps the click
    ...
    public SkyzOptionsScreen(Screen parent, GameOptions options) {
        super(FlowLayout.class, Identifier.of("skyz_client", "options"));
        this.parent  = parent;
    ...
    protected void build(FlowLayout root) {
        wire(root, "btn-back", () -> close(), SkyzButtonRenderer.NAV_BACK);
        ...
        // Header.
        LabelComponent header = UIComponents.label(Text.literal("⚙  GAME OPTIONS"))
        ...
        Object[][] rows = {
                {"🧍  SKIN CUSTOMIZATION",  (ButtonAction) () -> open(p -> new SkinOptionsScreen(p, options)),
                 "🔊  SOUND",                (ButtonAction) () -> open(p -> new SoundOptionsScreen(p, options))},
                {"🖥  VIDEO",                (ButtonAction) this::openVideoSettings,
                 "🎮  CONTROLS",             (ButtonAction) () -> open(p -> new ControlsOptionsScreen(p, options))},
                {"🌐  LANGUAGE",             (ButtonAction) () -> open(p -> new LanguageOptionsScreen(
                        p, options, MinecraftClient.getInstance().getLanguageManager())),
        ...
        ButtonComponent done = UIComponents.button(Text.literal("DONE"), b -> close())
    ...
    private FlowLayout buildRouteButton(String label, ButtonAction action) {
        ...
        ButtonComponent btn = UIComponents.button(Text.literal(label), b -> action.run())
    ...
    private void open(Function<Screen, Screen> screenFactory) {
        MinecraftClient.getInstance().setScreen(screenFactory.apply(this));
    }

    /**
     * Resource Packs sub-screen — vanilla {@link PackScreen} needs the
     ...
     * <ul>
     *   <li><b>Navigate back to ourselves.</b> {@code PackScreen.close()}
     ...
     *       Calling {@code reloadResources()} unconditionally triggers
    ...
    private void openResourcePacks() {
        ...
        mc.setScreen(new PackScreen(
                ...
                manager -> {
                    ...
                    // Always navigate back to ourselves — PackScreen.close()
     ...
     * neither installed, falls through to vanilla {@link VideoOptionsScreen}.
    ...
    private void openVideoSettings() {
        ...
        open(p -> new VideoOptionsScreen(p, MinecraftClient.getInstance(), options));
    }
    ...
    private FlowLayout buildShortcutButton(ShortcutDef s) {
        FlowLayout cell = UIContainers.horizontalFlow(Sizing.fill(49), Sizing.content());
        ButtonComponent btn = UIComponents.button(Text.literal(s.label), b -> s.action.run())
                .renderer(SkyzButtonRenderer.DEFAULT);
        btn.horizontalSizing(Sizing.fill(100));
        btn.verticalSizing(Sizing.fixed(22));
        cell.child(btn);
    ...
    public static boolean tryOpenSodiumOptions(Screen prev) {
        ...
        if (viaMM != null) {
            ...
            MinecraftClient.getInstance().setScreen(viaMM);
            return true;
        }
        ...
        for (String className : ctorCandidates) {
            try {
                ...
                MinecraftClient.getInstance().setScreen(sodiumScreen);
    ...
    public static boolean tryOpenIrisOptions(Screen prev) {
        ...
        try {
            ...
            MinecraftClient.getInstance().setScreen(iris);
            return true;
        } catch (Exception e) {
        ...
        for (String className : candidates) {
            try {
                ...
                try {
                    ...
                    MinecraftClient.getInstance().setScreen(dh);
                    return true;
                } catch (NoSuchMethodException ignored) {
                    // Try a no-arg ctor.
                    try {
