...
public class SkyzModConfigScreen extends Screen {
    ...
    private void drawNavBar(GuiGraphicsExtractor ctx) {
        int x = 10;
        ctx.text(font, "SKYZ", x, 10, 0xFFF0F8FF);
        x += font.width("SKYZ")+6;
        ctx.text(font, "/", x, 10, 0x4D8CD2FF);
        x += font.width("/")+6;
        ctx.text(font, "MODS", x, 10, 0x888CD2FF);
        x += font.width("MODS")+6;
        ctx.text(font, "/", x, 10, 0x4D8CD2FF);
        x += font.width("/")+6;
        ctx.text(font, modName.toUpperCase(), x, 10, 0x888CD2FF);
        SkyzRenderHelper.drawDivider(ctx, 0, NAV_H, width);
    }

    private void drawTabNav(GuiGraphicsExtractor ctx, int mx, int my) {
        int x = 10, y = NAV_H + 10;
        ctx.text(font, "SETTINGS", x, y, 0x2E8CD2FF);
        y += 14;
        for (int i = 0; i < TABS.length; i++) {
            ...
            ctx.text(font, TABS[i], x+10, y+6,
                    active ? SkyzColors.TEXT_PRIMARY : SkyzColors.TEXT_MUTED);
            y += 24;
        }
    }

    private void drawContent(GuiGraphicsExtractor ctx, int mx, int my) {
    ...
    private void drawGeneral(GuiGraphicsExtractor ctx, int x, int y, int w, int mx, int my) {
        switch (modName) {
    ...
    private void drawSodiumSettings(GuiGraphicsExtractor ctx, int x, int y, int w, int mx, int my) {
    ...
    private void drawIrisSettings(GuiGraphicsExtractor ctx, int x, int y, int w, int mx, int my) {
    ...
    private void drawModMenuSettings(GuiGraphicsExtractor ctx, int x, int y, int w, int mx, int my) {
    ...
    private void drawEssentialSettings(GuiGraphicsExtractor ctx, int x, int y, int w, int mx, int my) {
        ...
        ctx.text(font, "ℹ modrinth.com/mod/essential", x+4, y, 0x4D8CD2FF);
    }

    private void drawGenericSettings(GuiGraphicsExtractor ctx, int x, int y, int w, int mx, int my) {
    ...
    private void drawKeybinds(GuiGraphicsExtractor ctx, int x, int y, int w) {
        ...
        for (String[] b : new String[][]{{"Toggle Mod","K"},{"Open Config","None"},{"Reload","None"}}) {
            ...
            ctx.text(font, b[0], x+8, y+9, SkyzColors.TEXT_PRIMARY);
            int bw = font.width(b[1])+14;
            SkyzRenderHelper.fillPanel(ctx, x+w-bw-4, y+5, bw, 16, 0x33143C6E, 0x3A8CD2FF);
            ctx.text(font, b[1], x+w-bw+3, y+9, SkyzColors.TEXT_MUTED);
            y += ROW_H;
        }
    }

    private void drawAdvanced(GuiGraphicsExtractor ctx, int x, int y, int w) {
        drawSectionHeader(ctx, x, y, "Advanced"); y+=16;
        ctx.text(font, "Advanced options for " + modName + " v" + modVersion + ".", x+4, y, SkyzColors.TEXT_MUTED);
    }

    private void drawAbout(GuiGraphicsExtractor ctx, int x, int y, int w) {
    ...
    private void drawSectionHeader(GuiGraphicsExtractor ctx, int x, int y, String title) {
        ctx.text(font, title.toUpperCase(), x, y, 0x2E8CD2FF);
        SkyzRenderHelper.fillRect(ctx, x, y+10, font.width(title.toUpperCase())+20, 1, 0x1A8CD2FF);
    }

    private void drawToggleRow(GuiGraphicsExtractor ctx, int x, int y, int w,
                               String label, String sub, int idx, int mx, int my) {
        SkyzRenderHelper.fillRect(ctx, x, y, w, ROW_H-2, 0x1A091E46);
        ctx.text(font, label, x+8, y+(sub.isEmpty()?9:5), SkyzColors.TEXT_PRIMARY);
        if (!sub.isEmpty()) ctx.text(font, sub, x+8, y+15, 0x4D8CD2FF);
    ...
    private void drawSliderRow(GuiGraphicsExtractor ctx, int x, int y, int w,
                               String label, int min, int max, int idx, int mx, int my) {
        SkyzRenderHelper.fillRect(ctx, x, y, w, ROW_H-2, 0x1A091E46);
        ctx.text(font, label, x+8, y+9, SkyzColors.TEXT_PRIMARY);
        ...
        ctx.text(font, String.valueOf(val),
                sX - font.width(String.valueOf(val))-6, y+9, SkyzColors.TEXT_MUTED);
    }

    private void drawInfoRow(GuiGraphicsExtractor ctx, int x, int y, int w, String k, String v) {
        SkyzRenderHelper.fillRect(ctx, x, y, w, ROW_H-2, 0x1A091E46);
        ctx.text(font, k, x+8, y+9, SkyzColors.TEXT_MUTED);
        ctx.text(font, v, x+w-font.width(v)-8, y+9, SkyzColors.TEXT_PRIMARY);
    }
