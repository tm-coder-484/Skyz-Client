 ...
 * {@link net.minecraft.client.gui.screen.multiplayer.AddServerScreen} with the
 ...
 *   - A cycle button for {@link ServerInfo.ResourcePackPolicy}.
...
public class SkyzAddServerScreen extends BaseUIModelScreen<FlowLayout> {
    ...
    public SkyzAddServerScreen(Screen parent, Text title,
                               BooleanConsumer callback, ServerInfo server) {
        super(FlowLayout.class, Identifier.of("skyz_client", "add_server"));
        this.parent   = parent;
    ...
    protected void build(FlowLayout root) {
        ...
        if (titleLbl != null && title$ != null) {
            titleLbl.text(Text.literal(title$.getString().toUpperCase()));
        }
        ...
        if (addrField != null) {
            ...
            addrField.text(server.address == null ? "" : server.address);
        ...
        if (resourcePackBtn != null) {
            ...
            resourcePackBtn.onPress(b -> {
                ServerInfo.ResourcePackPolicy[] all = ServerInfo.ResourcePackPolicy.values();
    ...
    private void updateRpLabel() {
        ...
        ServerInfo.ResourcePackPolicy p = server.getResourcePackPolicy();
        resourcePackBtn.setMessage(Text.literal(
    ...
    private void addAndClose() {
        ...
        server.name    = n.isEmpty() ? Text.translatable("selectServer.defaultName").getString() : n;
        server.address = a;
