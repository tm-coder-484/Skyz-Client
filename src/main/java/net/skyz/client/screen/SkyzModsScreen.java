...
public class SkyzModsScreen extends BaseUIModelScreen<FlowLayout> {
    ...
    public SkyzModsScreen(SkyzTitleScreen parent) {
        super(FlowLayout.class, Identifier.of("skyz_client", "mods"));
        this.parent  = parent;
    ...
    private void rebuildModsList() {
        ...
        if (countLabel != null) {
            countLabel.text(Text.literal(visible.size() + " mod"
                    + (visible.size() == 1 ? "" : "s")));
        }

        if (visible.isEmpty()) {
            LabelComponent empty = UIComponents
                    .label(Text.literal("No mods match \"" + searchQuery + "\"."))
    ...
    private FlowLayout buildCard(ModContainer mod) {
        ...
        ButtonComponent cfg = UIComponents
                .button(Text.literal("⚙  CONFIG"), b -> openConfigFor(mod))
